...
variable "aws_region" {
  description = "AWS region for IAM policy conditions and derived ARNs."
  type        = string
  default     = "ap-south-1"
}
...
variable "role_path" {
  description = "IAM path for roles created by this root."
  type        = string
  default     = "/"
}

variable "permissions_boundary_arn" {
  description = "Optional permissions boundary ARN applied to all created roles."
  type        = string
  default     = null
}
...
variable "validate_existing_roles" {
  description = "When creation is disabled for a current-account role, look it up with aws_iam_role to fail fast if it is missing."
  type        = bool
  default     = true
}
...
variable "create_swg_read_role" {
  description = "Whether to create the SWG secret-read role in the current provider account. Set false for cross-account SWG."
  type        = bool
  default     = true
}

variable "create_swg_resource_access_role" {
  description = "Whether to create the SWG resource-access role in the current provider account. Set false for cross-account SWG."
  type        = bool
  default     = true
}
...
variable "external_swg_read_role_arn" {
  description = "Existing SWG secret-read role ARN to use when create_swg_read_role is false."
  type        = string
  default     = null
}

variable "external_swg_resource_access_role_arn" {
  description = "Existing SWG resource-access role ARN to use when create_swg_resource_access_role is false."
  type        = string
  default     = null
}
...
variable "attach_deploy_state_policy" {
  description = "Whether to attach Terraform state backend permissions to the deploy role when backend names or ARNs are supplied."
  type        = bool
  default     = true
}

variable "terraform_state_bucket_name" {
  description = "Optional S3 state bucket name used to derive a deploy-role state policy."
  type        = string
  default     = null
}

variable "terraform_lock_table_name" {
  description = "Optional DynamoDB state lock table name used to derive a deploy-role state policy."
  type        = string
  default     = null
}

variable "terraform_state_bucket_arns" {
  description = "Additional S3 state bucket ARNs the deploy role may access."
  type        = list(string)
  default     = []
}

variable "terraform_lock_table_arns" {
  description = "Additional DynamoDB lock table ARNs the deploy role may access."
  type        = list(string)
  default     = []
}

variable "terraform_state_key_prefixes" {
  description = "State object key prefixes allowed in state buckets. Empty defaults to environment/region/."
  type        = list(string)
  default     = []
}
...
variable "deploy_role_managed_policy_arns" {
  description = "Additional managed policy ARNs attached to the deploy role."
  type        = list(string)
  default     = []
}
...
variable "swg_read_managed_policy_arns" {
  description = "Additional managed policy ARNs attached to the SWG read role when created here."
  type        = list(string)
  default     = []
}

variable "swg_resource_access_managed_policy_arns" {
  description = "Additional managed policy ARNs attached to the SWG resource-access role when created here."
  type        = list(string)
  default     = []
}
...
variable "kms_key_arns" {
  description = "KMS key ARNs known at bootstrap time for decrypt/admin policies."
  type        = list(string)
  default     = []
}
...
variable "attach_secret_read_policy_to_swg_read_role" {
  description = "Whether to attach the generated Secrets Manager/KMS read policy to the SWG read role when created here."
  type        = bool
  default     = true
}

variable "attach_secret_read_policy_to_resource_access_role" {
  description = "Whether to attach the generated Secrets Manager/KMS read policy to the SWG resource-access role when created here."
  type        = bool
  default     = true
}
